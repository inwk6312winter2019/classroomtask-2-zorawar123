class IP():
	...
	def getmask(self):
		list1=[255,255,255,255]
		if 0 < self.mask <8:
			list1[0]=ip.convert(self.mask)
			return f"[{list1[0]}.0.0.0]"
		elif self.mask==8:
			return f"[255.0.0.0]"
		elif 8 < self.mask < 16:
			list1[1]=ip.convert(self.mask)
			return f"[{list1[0]}.{list1[1]}.0.0]"
		elif self.mask==16:
			return f"[255.255.0.0]"
		elif 16 < self.mask < 24:
			list1[2]=ip.convert(self.mask)
			return f"[{list1[0]}.{list1[1]}.{list1[2]}.0]"
		elif self.mask==24:
			return f"[255.255.255.0]"
		else:
			list1[3]=ip.convert(self.mask)
			return f"[{list1[0]}.{list1[1]}.{list1[2]}.{list1[3]}]"
